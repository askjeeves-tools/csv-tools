...
test("homepage has SEO meta tags and structured data", async ({ page }) => {
	...
	await expect(expander.locator("summary")).toHaveText("Supported CSV conversions");
	await expect(expander.locator("li")).toHaveCount(2);
	expect(await page.content()).toContain("CSV → JSON");
	...
	await expect(
		seoExpanders.filter({
			has: page.locator("summary", { hasText: "Supported CSV conversions" }),
		}),
	).toBeVisible();
	await expect(
...
test("static SEO files are served", async ({ request }) => {
	...
	expect(robotsText).toContain("Sitemap: https://csv.askjeeves.cc/sitemap-index.xml");
