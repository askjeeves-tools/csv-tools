# csv-tools UI & SEO Architecture Guide

> Adapted from the png-tools reference. In this repo, structured data is built by `buildCsvToolsExtraGraph()` in `src/seo-structured-data.ts`. Site origin: `https://csv.askjeeves.cc`.

Reference implementation for Ask Jeeves single-page converter sites. Use this document when scaffolding a sibling tool (e.g. `pdf.askjeeves.cc`, `webp.askjeeves.cc`).
...
`buildPngToolsExtraGraph(siteOrigin, featureList)` returns 5 linked `@graph` entities:
...
| `public/favicon.ico`, `apple-touch-icon.png`, `favicon-16x16.png` | Copied from `public/images/icons/` by `scripts/generate-seo-images.mjs` |
...
8. **`src/seo-structured-data.ts`** — copy `buildPngToolsExtraGraph` pattern; rename for tool; adjust `applicationSubCategory`, HowTo name
